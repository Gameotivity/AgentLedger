...
class WasteFlag(Base):
    ...
    agent_name: Mapped[str] = mapped_column(String(255), nullable=False)
    task_name: Mapped[str | None] = mapped_column(String(255))
    waste_type: Mapped[str] = mapped_column(String(50), nullable=False)  # retry_loop | over_qualified_model | context_bloat | idle_heartbeat
...
class Budget(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    project_id: Mapped[str] = mapped_column(String(255), nullable=False)
    agent_name: Mapped[str | None] = mapped_column(String(255))  # null = project-wide
